...
from ui.components.executive_summary import render_executive_summary
from analytics.alert_analytics import AlertAnalytics
from analytics.forecast_analytics import ForecastAnalytics
...
.js-plotly-plot, .plotly, .plot-container { width: 100% !important; min-width: 0 !important; }
div[style*="overflow-y:auto"] { -webkit-overflow-scrolling: touch; overflow-x: auto !important; }
...
@media (max-width: 768px) {
    ...
    .kpi-card { padding: 0.65rem 0.85rem !important; min-height: 70px !important; gap: 0.5rem !important; }
    .kpi-icon { width: 32px !important; height: 32px !important; font-size: 1rem !important; }
    .kpi-value { font-size: 0.95rem !important; }
    .kpi-label { font-size: 0.62rem !important; }
    .kpi-delta-pos, .kpi-delta-neg { font-size: 0.62rem !important; }
...
@media (max-width: 480px) {
    ...
    .kpi-card  { gap: 0.4rem !important; }
    .block-container { padding: 0.4rem 0.25rem !important; }
    [data-testid="stHorizontalBlock"] > [data-testid="stColumn"] {
        width: 50% !important; min-width: 50% !important; flex: 0 0 50% !important;
    }
}

[data-testid="stSidebar"] { background: #0a1120 !important; }
...
.kpi-icon {
    font-size: 1.8rem; background: #1e3a5f; border-radius: 10px;
    width: 52px; height: 52px; display: flex; align-items: center;
    justify-content: center; flex-shrink: 0;
}
.kpi-value { font-size: 1.6rem; font-weight: 700; color: #f1f5f9; margin: 0; line-height: 1.2; }
.kpi-label { font-size: 0.72rem; color: #64748b; margin: 0; text-transform: uppercase; letter-spacing: 0.08em; }
...
with st.sidebar:
    st.markdown(
        "<p style='color:#f1f5f9;font-size:1.1rem;font-weight:700;margin:0 0 0.2rem 0;'>🤖 SalesBot</p>"
...
with h_left:
    st.markdown(
        ...
        "<div>"
        "<h2 style='color:#f1f5f9;margin:0;font-size:1.6rem;font-weight:700;display:inline;'>Dashboard</h2>"
...
try:
    ...
    cat  = category_analytics.revenue_by_category(province=province_filter, year=year_filter)
    trend_data = trend_analytics.monthly_revenue(province=province_filter, year=year_filter)

    if month_filter is not None:
        daily_data = trend_analytics.daily_revenue(
            month=month_filter, year=year_filter, province=province_filter
        )
        df_daily = pd.DataFrame(daily_data["data"])
        for col in df_daily.columns:
            try: df_daily[col] = pd.to_numeric(df_daily[col])
    ...
    alerts = alert_analytics.get_all_alerts(year=year_filter, province=province_filter)

    # Forecast — skip kalau month filter aktif (tidak relevan)
    if month_filter is None:
        forecast_analytics = ForecastAnalytics()
        forecast_result = forecast_analytics.forecast_next_months(n_months=3, province=province_filter)
        if forecast_result["status"] != "success":
            forecast_result = forecast_analytics.forecast_simple(n_months=3, province=province_filter)
    else:
        forecast_result = {"status": "hidden", "forecasts": []}
    ...
    alerts = []
    forecast_result = {"status": "error", "forecasts": []}

# Export
with export_placeholder:
    csv_data = df_prov_display.to_csv(index=False) if data_loaded else "no data"
    st.download_button("⬇ Export", data=csv_data,
...
for col, icon, label, value, delta, delta_label in [
    ...
    with col:
        if delta is not None:
            ...
            delta_html = (f"<p style='font-size:0.78rem;color:{color};margin:4px 0 0 0;'>"
                          f"{arrow} {abs(delta):.1f}% {delta_label}</p>")
        else:
            delta_html = (f"<p style='font-size:0.72rem;color:#475569;margin:4px 0 0 0;'>"
                          f"{delta_label}</p>")
        st.markdown(
            "<div class='kpi-card'>"
...
if alerts:
    ...
    if danger_alerts or warning_alerts or success_alerts:
        st.markdown(
            "<p style='color:#f1f5f9;font-size:1.1rem;font-weight:600;"
            "margin:0 0 0.5rem 0;'>🔔 Anomaly Detection</p>",
            unsafe_allow_html=True
        )
        for alert in (danger_alerts + warning_alerts + success_alerts)[:6]:
            if alert["type"] == "danger":
                color, bg, border = "#ef4444", "rgba(239,68,68,0.08)", "rgba(239,68,68,0.3)"
            elif alert["type"] == "warning":
                color, bg, border = "#f59e0b", "rgba(245,158,11,0.08)", "rgba(245,158,11,0.3)"
            else:
                color, bg, border = "#22c55e", "rgba(34,197,94,0.08)", "rgba(34,197,94,0.3)"
            st.markdown(
                f"<div style='background:{bg};border:1px solid {border};"
                f"border-left:3px solid {color};border-radius:10px;"
                f"padding:0.65rem 1rem;margin-bottom:0.4rem;display:flex;"
                f"align-items:center;gap:0.75rem;'>"
                f"<span style='font-size:1rem;'>{alert['icon']}</span>"
                f"<p style='margin:0;color:#e2e8f0;font-size:0.88rem;'>{alert['message']}</p>"
                f"</div>",
                unsafe_allow_html=True
            )
        st.markdown("<br>", unsafe_allow_html=True)
...
render_executive_summary(
    df_prov_display, df_cat, df_trend, df_products,
        ...
        if not df_table.empty:
            st.markdown(html_table(df_table, cols_show,
                                   {"province": "Province", "Revenue (Rp)": "Revenue",
                                    "Profit (Rp)": "Profit", "Units Sold": "Units",
                                    "Margin": "Margin"}),
                        unsafe_allow_html=True)
        else:
...
def _fmt_products(df):
    d = df.copy()
    for col, fmt in [("total_revenue", "Rp {:,.0f}"), ("total_profit", "Rp {:,.0f}"),
                     ("total_units", "{:,.0f}"), ("profit_margin_pct", "{:.1f}%")]:
        if col in d.columns:
            if "Rp" in fmt:
                d[col] = d[col].apply(lambda x: fmt.format(x))
            else:
                d[col] = d[col].apply(lambda x: fmt.format(x))
    return d
...
prod_labels = {"product_name": "Product", "category": "Category",
               "total_revenue": "Revenue", "total_units": "Units",
               "profit_margin_pct": "Margin"}
prov_labels = {"province": "Province", "Revenue (Rp)": "Revenue",
...
if province_filter is not None:
    ...
    if not df_products.empty:
        st.markdown(html_table(_fmt_products(df_products), prod_cols, prod_labels, max_rows=10),
                    unsafe_allow_html=True)
    else:
    ...
    with right:
        st.markdown("<p style='color:#f1f5f9;font-size:1.1rem;font-weight:600;"
                    ...
                    unsafe_allow_html=True)
        if not df_table.empty:
            st.markdown(html_table(df_table, cols_show, prov_labels), unsafe_allow_html=True)
        else:
...
if month_filter is None:
  st.markdown(
      "<p style='color:#f1f5f9;font-size:1.1rem;font-weight:600;"
      "margin:0 0 0.75rem 0;'>📈 Forecasting Revenue</p>",
      unsafe_allow_html=True
  )
...
if forecast_result["status"] == "success" and forecast_result["forecasts"]:
    ...
    for fcol, fc in zip(fcols, forecasts):
        with fcol:
            st.markdown(
                f"<div style='background:#1e293b;border:1px solid #334155;"
                f"border-radius:12px;padding:1rem;text-align:center;'>"
                f"<p style='color:#64748b;font-size:0.72rem;text-transform:uppercase;"
                f"letter-spacing:0.08em;margin:0;'>{fc['label']}</p>"
                f"<p style='color:#f1f5f9;font-size:1.3rem;font-weight:700;"
                f"margin:0.4rem 0 0.2rem;'>Rp {fc['predicted_revenue']/1e9:.2f}B</p>"
                f"<p style='color:#475569;font-size:0.75rem;margin:0;'>"
                f"Range: Rp {fc['lower']/1e9:.2f}B – Rp {fc['upper']/1e9:.2f}B</p>"
                f"</div>",
                unsafe_allow_html=True
            )
    st.markdown(
        f"<p style='color:#334155;font-size:0.72rem;margin:0.5rem 0 0 0;'>"
        f"Model: {forecast_result.get('model_info','')}</p>",
        unsafe_allow_html=True
    )
else:
    st.markdown(
        f"<div style='background:#1e293b;border:1px solid #334155;border-radius:12px;"
        f"padding:1rem;color:#475569;font-size:0.9rem;'>"
        f"Forecasting tidak tersedia: {forecast_result.get('message','')}</div>",
        unsafe_allow_html=True
    )
...
if not df_prov_display.empty:
    ...
    html = (
        ...
        "Provinsi terbaik:&nbsp;<span style='color:#f1f5f9;font-weight:600;'>" + v_best +
        "</span>&nbsp;·&nbsp;Revenue&nbsp;<span style='color:#38bdf8;font-weight:600;'>" + v_brev +
        "</span>&nbsp;·&nbsp;Margin&nbsp;<span style='color:#34d399;font-weight:600;'>" + v_marg +
        "</span></p></div>"
        "<div style='display:flex;gap:0.75rem;'><span>🏷️</span>"
        "<p style='margin:0;color:#cbd5e1;font-size:0.93rem;line-height:1.6;'>"
        "Kategori terlaris:&nbsp;<span style='color:#f1f5f9;font-weight:600;'>" + v_cat +
        "</span>&nbsp;·&nbsp;kontribusi&nbsp;<span style='color:#38bdf8;font-weight:600;'>" + v_cpct +
        "</span>&nbsp;dari total revenue</p></div>"
        "<div style='display:flex;gap:0.75rem;'><span>📉</span>"
        "<p style='margin:0;color:#cbd5e1;font-size:0.93rem;line-height:1.6;'>"
        "Provinsi terendah:&nbsp;<span style='color:#f1f5f9;font-weight:600;'>" + v_worst +
        "</span>&nbsp;·&nbsp;Revenue&nbsp;<span style='color:#f87171;font-weight:600;'>" + v_wrev +
        "</span>&nbsp;· perlu perhatian khusus</p></div>"
        "</div>"
    ...
    st.markdown(
        "<div style='background:#1e293b;border:1px solid #334155;"
        "border-radius:14px;padding:1.2rem 1.6rem;color:#475569;font-size:0.9rem;'>"
